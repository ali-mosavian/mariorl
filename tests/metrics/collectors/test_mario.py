...
@pytest.fixture
def mario_collector(mock_logger):
...
class TestMarioCollectorOnStep:
    """Tests for on_step method."""
    
    def test_tracks_x_pos(self, mario_collector, mock_logger):
        """Should track x_pos as gauge."""
        mario_collector.on_step({"x_pos": 150})
        mock_logger.gauge.assert_called_with("x_pos", 150)
    
    def test_tracks_game_time_from_nested_state(self, mario_collector, mock_logger):
        """Should extract game_time from nested state dict."""
        mario_collector.on_step({
            "x_pos": 100,
            "state": {"time": 380}
        })
    ...
    def test_updates_best_x_tracking(self, mario_collector, mock_logger):
...
class TestMarioCollectorOnEpisodeEnd:
    """Tests for on_episode_end method."""
    
    def test_counts_death_when_is_dead(self, mario_collector, mock_logger):
    ...
    def test_counts_death_when_is_dying(self, mario_collector, mock_logger):
        """Should increment deaths counter when is_dying."""
        mario_collector.on_episode_end({"is_dying": True})
        mock_logger.count.assert_called_with("deaths")
    
    def test_counts_flag_when_captured(self, mario_collector, mock_logger):
    ...
    def test_flag_takes_precedence_over_death(self, mario_collector, mock_logger):
    ...
    def test_calculates_speed_from_position_and_time(self, mario_collector, mock_logger):
    ...
    def test_resets_episode_tracking_after_end(self, mario_collector, mock_logger):
    ...
    def test_no_speed_when_no_time_elapsed(self, mario_collector, mock_logger):
        ...
        speed_calls = [c for c in mock_logger.observe.call_args_list 
                      if c[0][0] == "speed"]
        assert len(speed_calls) == 0


class TestMarioCollectorOnTrainStep:
    """Tests for on_train_step method."""
    
    def test_ignores_train_metrics(self, mario_collector, mock_logger):
...
class TestMarioCollectorFlush:
    """Tests for flush method."""
    
    def test_flushes_logger(self, mario_collector, mock_logger):
        """Should call flush on the logger."""
        mario_collector.flush()
        mock_logger.flush.assert_called_once()


class TestMarioCollectorProtocol:
    """Tests that MarioCollector satisfies MetricCollector protocol."""
    
    def test_satisfies_protocol(self, mario_collector):
