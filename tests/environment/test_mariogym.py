...
from mario_rl.environment import TIMEOUT_THRESHOLD  # type: ignore[attr-defined]
...
class TestTimeoutThreshold:
    ...
    def test_timeout_threshold_value(self) -> None:
        ...
        from mario_rl.environment.mariogym import TIMEOUT_THRESHOLD as DIRECT_IMPORT  # type: ignore[attr-defined]
...
class TestIsTimeoutFlag:
    ...
    def test_is_timeout_true_when_dead_and_time_low(self) -> None:
        ...
        """
        with (
            patch.object(MarioBrosLevel, "_is_dead", new_callable=PropertyMock, return_value=True),
            patch.object(MarioBrosLevel, "_is_dying", new_callable=PropertyMock, return_value=False),
            patch.object(MarioBrosLevel, "_time", new_callable=PropertyMock, return_value=5),
            patch.object(MarioBrosLevel, "_flag_get", new_callable=PropertyMock, return_value=False),
            patch.object(MarioBrosLevel, "_reward_state", new_callable=PropertyMock),
            patch.object(MarioBrosLevel, "state", new_callable=PropertyMock),
            patch.object(MarioBrosLevel, "_last_state", None),
        ):
    ...
    def test_is_timeout_true_when_dying_and_time_low(self) -> None:
        ...
        with (
            patch.object(MarioBrosLevel, "_is_dead", new_callable=PropertyMock, return_value=False),
            patch.object(MarioBrosLevel, "_is_dying", new_callable=PropertyMock, return_value=True),
            patch.object(MarioBrosLevel, "_time", new_callable=PropertyMock, return_value=10),
            patch.object(MarioBrosLevel, "_flag_get", new_callable=PropertyMock, return_value=False),
        ):
            env = object.__new__(MarioBrosLevel)
    ...
    def test_is_timeout_false_when_time_above_threshold(self) -> None:
        ...
        with (
            patch.object(MarioBrosLevel, "_is_dead", new_callable=PropertyMock, return_value=True),
            patch.object(MarioBrosLevel, "_is_dying", new_callable=PropertyMock, return_value=False),
            patch.object(MarioBrosLevel, "_time", new_callable=PropertyMock, return_value=100),
            patch.object(MarioBrosLevel, "_flag_get", new_callable=PropertyMock, return_value=False),
        ):
            env = object.__new__(MarioBrosLevel)
    ...
    def test_is_timeout_false_when_flag_reached(self) -> None:
        ...
        with (
            ...
            patch.object(MarioBrosLevel, "_is_dying", new_callable=PropertyMock, return_value=False),
            patch.object(MarioBrosLevel, "_time", new_callable=PropertyMock, return_value=5),
            patch.object(MarioBrosLevel, "_flag_get", new_callable=PropertyMock, return_value=True),
        ):
            env = object.__new__(MarioBrosLevel)
    ...
    def test_is_timeout_at_exact_threshold(self) -> None:
        ...
        with (
            patch.object(MarioBrosLevel, "_is_dead", new_callable=PropertyMock, return_value=True),
            patch.object(MarioBrosLevel, "_is_dying", new_callable=PropertyMock, return_value=False),
            patch.object(MarioBrosLevel, "_time", new_callable=PropertyMock, return_value=TIMEOUT_THRESHOLD),
            patch.object(MarioBrosLevel, "_flag_get", new_callable=PropertyMock, return_value=False),
        ):
            env = object.__new__(MarioBrosLevel)
    ...
    def test_is_timeout_just_above_threshold(self) -> None:
        ...
        with (
            patch.object(MarioBrosLevel, "_is_dead", new_callable=PropertyMock, return_value=True),
            patch.object(MarioBrosLevel, "_is_dying", new_callable=PropertyMock, return_value=False),
            patch.object(MarioBrosLevel, "_time", new_callable=PropertyMock, return_value=TIMEOUT_THRESHOLD + 1),
            patch.object(MarioBrosLevel, "_flag_get", new_callable=PropertyMock, return_value=False),
        ):
            env = object.__new__(MarioBrosLevel)
