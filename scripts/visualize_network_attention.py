    ...
    for y in range(0, H - patch_size + 1, stride):
        for x in range(0, W - patch_size + 1, stride):
            ...
            sensitivity[y : y + patch_size, x : x + patch_size] = np.maximum(
                sensitivity[y : y + patch_size, x : x + patch_size], q_change
            )
...
def create_attention_visualization(
    ...
    if danger_pred is not None and len(danger_pred) > 0:
        ...
        ax.bar(range(bins), danger_pred, color=colors, edgecolor="black", linewidth=0.5)
...
def main():
    ...
    parser.add_argument(
        "--checkpoint", type=str, default="checkpoints/ddqn_dist_2026-01-15T21-58-18/checkpoint_310000.pt"
    )
