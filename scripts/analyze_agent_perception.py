...
from mario_rl.agent.ddqn_net import DDQNNet, DoubleDQN
...
def load_model(checkpoint_path: Path, device: str = "cuda") -> DDQNNet:
    ...
    print(f"  Detected {'DoubleDQN' if is_double_dqn else 'DDQNNet'} with {in_channels} channels, {input_size}x{input_size} input")
...
def create_diagnostic_image(
    ...
    ax_raw.text(
        10, 230, f"X: {record.x_pos}", fontsize=10, color="white",
    ...
    for bar, val in zip(bars, record.q_values):
        ax_q.text(
    ...
    for i in range(4):
        ax_frames.text(
            i * 84 + 42, 5, f"t-{3-i}", fontsize=9, color="red",
    ...
    ax_analysis.text(
        0.05, 0.95, analysis_text, transform=ax_analysis.transAxes,
...
def run_analysis(
    ...
    for episode in range(num_episodes):
        ...
        while not done and step < max_steps_per_episode:
            ...
            should_save = (
                step % save_every_n == 0 or
                record.scenario in ["DEATH", "STUCK", "BACKWARD", "LOW_CONFIDENCE"] or
                (step > 0 and step % 50 == 0)
            )
            
            if should_save:
                img_path = output_dir / f"ep{episode:02d}_step{step:05d}_{record.scenario}.png"
                create_diagnostic_image(
                    record, img_path, episode,
                    prev_records=episode_records[:-1] if len(episode_records) > 1 else None,
                )
                print(f"  Step {step:5d} | X={record.x_pos:5d} | {record.scenario:15s} | Q-range={np.ptp(q_values):.1f}")
        ...
        if episode_records and episode_records[-1].is_death:
            ...
            create_diagnostic_image(
                episode_records[-1], img_path, episode,
...
def create_summary(records: list[FrameRecord], output_dir: Path):
    ...
    bars = ax.bar(scenario_names, scenario_means, yerr=scenario_stds, capsize=5)
