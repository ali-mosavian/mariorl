        ...
        with col_rates:
            if level in level_rate_data and len(level_rate_data[level]) >= 2:
                ...
                for d in sampled:
                    ...
                    # Deaths per episode (can be >1 due to 3 lives + snapshot restores)
                    deaths_per_ep = (deaths / episodes) if episodes > 0 else 0
                    # Completion rate = flags / episodes (as percentage, 0-100%)
                    completion_rate = (flags / episodes * 100) if episodes > 0 else 0
                    
                    death_rates.append(deaths_per_ep)
                ...
                fig.add_trace(go.Scatter(
                    ...
                    hovertemplate="Steps: %{x}<br>Deaths/Episode: %{y:.2f}<extra></extra>",
                ), secondary_y=False)
                ...
                fig.update_yaxes(title_text="Deaths/Ep", secondary_y=False, gridcolor="#313244", range=[0, death_max], title_font=dict(color="crimson"))
                ...
                # Current rates
                st.caption(f"💀 Deaths/Ep: {death_rates[-1]:.2f} | 🏁 Complete: {completion_rates[-1]:.1f}%")
            else:
