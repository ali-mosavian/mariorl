...
def compute_motion_history(frames: list[np.ndarray], decay: float = 0.9) -> list[np.ndarray]:
    ...
    for i in range(1, len(frames)):
        ...
        histories.append(
            (history / history.max() * 255).astype(np.uint8) if history.max() > 0 else history.astype(np.uint8)
        )
...
def create_summary_plot(
    ...
    axes[1, 0].set_title("Pixel Variance Map\n(brighter = more change)")
    ...
    axes[1, 2].text(
        0.05, 0.95, f"Correlation: {corr:.3f}", transform=axes[1, 2].transAxes, fontsize=10, verticalalignment="top"
    )
...
@click.command()
@click.option("-c", "--checkpoint", type=click.Path(exists=True), required=True, help="Path to DDQN checkpoint")
@click.option("-f", "--frames", default=50, help="Number of frames to collect")
@click.option("-s", "--skip", default=4, help="Frame skip interval")
@click.option(
    "-o", "--output", type=click.Path(), default="/tmp/frame_representations.png", help="Output path for visualization"
)
