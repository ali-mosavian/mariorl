...
def create_sequence_image(
    ...
    fig.suptitle(
        f"Episode {episode} | {event_type} at X={last.x_pos} | Last {n_show} frames before event",
        fontsize=16,
        fontweight="bold",
        color=color,
    )
    ...
    # Mark chosen actions
    for i, frame in enumerate(frames_to_show):
        ax_ts.scatter(i, frame.q_values[frame.chosen_action], c="black", s=100, zorder=5, marker="*")
    ...
    for episode in range(num_episodes):
        ...
        while step < max_steps:
            ...
            if step % 200 == 0:
                ...
                print(
                    f"  Step {step:5d} | X={x_pos:5d} | {mode_str:6s} | Action={ACTION_NAMES[action]:6s} | Q-range={q_range:.2f}"
                )
