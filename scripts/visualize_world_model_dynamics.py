...
def visualize_dynamics_comparison(
    ...
    for real, imagined in zip(real_frames, imagined_frames):
    ...
    for col, idx in enumerate(indices):
        ...
        axes[1, col].set_title(f"Imagined", fontsize=8)
        axes[1, col].axis("off")
...
def compute_dynamics_metrics(
    ...
    for t, (real, imagined) in enumerate(zip(real_frames, imagined_frames)):
        real_norm = real / 255.0
        mse = np.mean((real_norm - imagined) ** 2)
